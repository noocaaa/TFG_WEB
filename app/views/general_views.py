...
@general_blueprint.route('/juegos')
@login_required
def games():
    # Comprobar si el usuario está loggeado
    if not current_user.is_authenticated:  
    ...
    return render_template('juegos.html', user=current_user)

@general_blueprint.route('/juegos/ajedrez')
@login_required
def chess():
    # Comprobar si el usuario está loggeado
    if not current_user.is_authenticated:  
...
@general_blueprint.route('/juegos/2048')
@login_required
def g_2048():
    # Comprobar si el usuario está loggeado
    if not current_user.is_authenticated:  
...
@general_blueprint.route('/juegos/hextris')
@login_required
def hextris():
    # Comprobar si el usuario está loggeado
    if not current_user.is_authenticated:  
    ...
    return render_template('hextris.html', user=current_user)

@general_blueprint.route('/juegos/minesweeper')
@login_required
def minesweeper():
    # Comprobar si el usuario está loggeado
    if not current_user.is_authenticated:  
    ...
    return render_template('minesweeper.html', user=current_user)